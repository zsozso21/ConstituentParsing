...
//	}
	...
	public void splitRule(int nSubstates, Random random){
		splitRuleHelper(hierarchy, 2, random);
...
//		mapping[1] = (short)1;
	...
	private void splitRuleHelper(Tree<Double> tree, int splitFactor, Random random) {
		if (tree.isLeaf()){
			if (tree.getLabel()!=0||nParam==1){ // split it
				ArrayList<Tree<Double>> children = new ArrayList<Tree<Double>>(splitFactor);
				for (int i=0; i<splitFactor; i++){
					Tree<Double> child = new Tree<Double>((random.nextDouble()-.5)/100.0);
					children.add(child);
				}
...
//				tree.setLabel(GrammarTrainer.RANDOM.nextDouble()/100.0);
			...
			for (Tree<Double> child : tree.getChildren()){
				splitRuleHelper(child, splitFactor, random);
			}
		}
	}
