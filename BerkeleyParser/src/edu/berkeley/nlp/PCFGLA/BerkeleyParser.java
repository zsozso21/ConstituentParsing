...
public class BerkeleyParser  {
  ...
  @SuppressWarnings("unchecked")
    ...
    if(lexicon instanceof LatticeLexicon)
    	((LatticeLexicon)lexicon).setTagNumberer((Numberer)pData.getNumbs().get("tags"));
    
    if (opts.chinese) Corpus.myTreebank = Corpus.TreeBankType.CHINESE;
    
    CoarseToFineMaxRuleParser parser = null;
    if (opts.kbest==1) parser = new CoarseToFineMaxRuleParser(grammar, lexicon, pData.getNumbs(), threshold,-1,opts.viterbi,opts.substates,opts.scores, opts.accurate, false, true, true);
    else parser = new CoarseToFineNBestParser(grammar, lexicon, pData.getNumbs(), opts.kbest,threshold,-1,opts.viterbi,opts.substates,opts.scores, opts.accurate, false, false, true);
          ...
          while (m_parser.hasNext()){
          	List<Tree<String>> parsedTrees = m_parser.getNext();
    			...
    			if (opts.kbest > 1){
    	  		...
    	  		if(parsedTree.getChildren().isEmpty()){ // create dummy "parse" 
	  				...
	  				parsedTree.setChildren(preterminal_list);
    	  		}
