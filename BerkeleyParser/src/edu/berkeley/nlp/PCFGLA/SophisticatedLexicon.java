...
public class SophisticatedLexicon implements java.io.Serializable, Lexicon {
  ...
  public double totalTokens = 0.0;
  public double totalUnseenTokens = 0.0;
...
//  	}
  ...
  @SuppressWarnings("unchecked")
	...
	public SophisticatedLexicon(int uwm, short[] numSubStates, int smoothingCutoff, double[] smoothParam, 
			Smoother smoother, double threshold, Numberer wnum) {
		this(numSubStates, smoothingCutoff, smoothParam, smoother, threshold,wnum);
		unknownLevel = uwm;
	}
...
//			}
		...
		SophisticatedLexicon lexicon = new LatticeLexicon(newNumSubStates, this.smoothingCutoff, smooth, smoother, this.threshold, this.wordNumberer);
...
/*          	if (Corpus.myLanguage==Corpus.GERMAN){
      ...
      case 6:
      {
        ...
        if (wlen >= 3) {
        	String suffixes="";
        	if(sb.toString().equals("UNK-CAPS"))
        		suffixes="t,ft,äft,rft,it,ät,tät,ität,dt,gt,ßt,n,en,hen,ien,rn,ern,uern,lern,zern,tzern,nzern,onzern,ln,eln,an,nn,ann,mann,inn,e,he,ae,ce,je,s,us,is,kis,as,os,ws,ss,ps,h,th,a,ma,r,är,ir,ar,mar,hr,y,d,ud,l,k,f,ef,o,c,u,i,ei,kei,w,ß,j";
        	else if(sb.toString().equals("UNK-LC"))
        		suffixes="e,he,che,uche,eche,ahe,ehe,ve,ive,ne,ene,de,ce,n,en,ben,fen,ven,iven,an,hn,in,on,t,h,ch,s,es,os,los,ös,r,er,ar,bar,or,hr,m,em,a,o,y,w";
        	else if(sb.toString().equals("UNK-CAPS-DASH"))
        		suffixes="r,us,is,s,f,ß";
        	else if(sb.toString().equals("UNK-LC-DASH"))
        		suffixes="e";
        	String longestsuffix = "";
        	for(String suffix : suffixes.split(","))
	        	if (lowered.endsWith(suffix) && longestsuffix.length()<suffix.length()) {
	        		longestsuffix=suffix;
	          }
        	if(longestsuffix.length()>0){
        		System.out.println("ls:"+longestsuffix);
...
*/	
   ...
   */
  public void trainTree(Tree<StateSet> trainTree, double randomness, Lexicon oldLexicon, boolean secondHalf, boolean noSmoothing, Random random) {
  	// scan data
    ...
    int sentenceScale = trainTree.getLabel().getIScale();
      ...
      for (short substate=0; substate<nSubStates; substate++) {
        ...
        if (randomness == -1) {
          ...
          weight = random.nextDouble()*randomness/100.0+1.0;
        }
        if (weight==0)
        	continue;
...
//			for (int substate=0; substate<numSubStates[tag]; substate++) {
  ...
  public void logarithmMode() {
	...
	public SophisticatedLexicon projectLexicon(double[] condProbs, int[][] mapping, int[][] toSubstateMapping) {
		...
		SophisticatedLexicon newLexicon = new LatticeLexicon(newNumSubStates, this.smoothingCutoff, this.smooth, newSmoother, this.threshold, this.wordNumberer);
...
//		newLexicon.smoother = smoother; 
	...
	public SophisticatedLexicon copyLexicon() {
		...
		SophisticatedLexicon newLexicon = new LatticeLexicon(newNumSubStates, this.smoothingCutoff, this.smooth, this.smoother, this.threshold, this.wordNumberer);
