...
//    int lang = opts.lang;
    ...
    //List<Tree<String>> validationTrees = BerkeleyParserWrapper.readCorpus("/mount/corpora12/d5/statmt/farkas/english_reranking/corpora/sec1.parses");
...
//    	startSplit = 1; // we've already trained the grammar
      ...
      tagNumberer =  pData.gr.tagNumberer;
      wordNumberer = ((SophisticatedLexicon)pData.lex).wordNumberer;
    ...
    if (splitGrammarFile==null) {
    	grammar = new Grammar(numSubStatesArray, findClosedUnaryPaths, new NoSmoothing(), null, filter, tagNumberer);
			lexicon = (opts.simpleLexicon) ? 
					...
					new LatticeLexicon(opts.unknownLevel,numSubStatesArray,SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF,smoothParams, new NoSmoothing(),filter, wordNumberer, tagNumberer);
			int n = 0;
			boolean secondHalf = false;
			for (Tree<StateSet> stateSetTree : trainStateSetTrees) {
				secondHalf = (n++>nTrees/2.0); 
				lexicon.trainTree(stateSetTree, randomness, null, secondHalf,false, GrammarTrainer.RANDOM);
				grammar.tallyUninitializedStateSetTree(stateSetTree);
			}
			lexicon.optimize();
			grammar.optimize(randomness, GrammarTrainer.RANDOM);
...
//        Smoother lexSmoother = new SmoothAcrossParentSubstate(0.1);
        ...
        opString = "smoothing";
    		...
    		maxGrammar = maxGrammar.splitAllStates(randomness, counts, allowMoreSubstatesThanCounts, 0, GrammarTrainer.RANDOM);
        ...
        minIterations = opts.splitMinIterations;
    		...
    		lexicon = (opts.simpleLexicon) ? 
  					...
  					new LatticeLexicon(opts.unknownLevel,newNumSubStatesArray,SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, maxLexicon.getSmoothingParams(), maxLexicon.getSmoother(), maxLexicon.getPruningThreshold(),wordNumberer,tagNumberer);
...
//    		System.out.println("The training LL is "+trainingLikelihood);
    	...
    	do {
    		...
    		System.out.println("Beginning iteration "+(iter-1)+":");
  			...
  			lexicon = (opts.simpleLexicon) ? 
  					...
  					new LatticeLexicon(opts.unknownLevel,grammar.numSubStates,	SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, lexicon.getSmoothingParams(), lexicon.getSmoother(), lexicon.getPruningThreshold(),wordNumberer,tagNumberer);
  			...
  			grammar.optimize(0, GrammarTrainer.RANDOM); // M Step
    	...
    	ParserData pData = new ParserData(maxLexicon, maxGrammar, null, numbererMap, numSubStatesArray, VERTICAL_MARKOVIZATION, HORIZONTAL_MARKOVIZATION, binarization);
	...
	public static double doOneEStep(Grammar previousGrammar, Lexicon previousLexicon, Grammar grammar, Lexicon lexicon, StateSetTreeList trainStateSetTrees,
		...
		for (Tree<StateSet> stateSetTree : trainStateSetTrees) {
			...
			if ((Double.isInfinite(ll) || Double.isNaN(ll))) {
			  ...
			  lexicon.trainTree(stateSetTree, -1, previousLexicon, secondHalf,noSmoothing, GrammarTrainer.RANDOM);
