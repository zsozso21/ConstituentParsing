...
public class BerkeleyParserWrapper {
	...
	public static double calcConfidence(CoarseToFineMaxRuleParser parser, Tree<String> tree) {
		try{
			return (tree.getChildren().isEmpty()) ? Double.NEGATIVE_INFINITY : parser.getLogLikelihood(tree);
		} catch (Exception e) {
	...
	public static ParserData train(List<Tree<String>> trainTrees, int randSeed, CoarseToFineNBestParser parser){
		...
		GrammarTrainer.Options opts = new GrammarTrainer.Options(); // gives the default option values from GrammarTrainer  
...
//  If we load the old grammar we don't split
	  if (parser!=null) {
	  	startSplit = 2;
	    tagNumberer = parser.getTagNumberer();
	    wordNumberer = parser.getWordNumberer();
//	  	grammar = parser.getGrammar();
//	  	maxGrammar = new Grammar(grammar.numSubStates, grammar.findClosedPaths, grammar.smoother, grammar, grammar.threshold, tagNumberer);
	  	maxGrammar = parser.getGrammar().copyGrammar(true);
//	  	lexicon = parser.getLexicon();
//	  	maxLexicon = new SophisticatedLexicon(opts.unknownLevel,grammar.numSubStates,	SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, lexicon.getSmoothingParams(), lexicon.getSmoother(), lexicon.getPruningThreshold(),wordNumberer);
	  	maxLexicon = parser.getLexicon().copyLexicon();
	  	numSubStatesArray = Arrays.copyOf(maxGrammar.numSubStates, maxGrammar.numSubStates.length);
	  	...
	  	trainStateSetTrees = new StateSetTreeList(trainTrees, numSubStatesArray, false, tagNumberer);
			...
			for (Tree<StateSet> stateSetTree : trainStateSetTrees) {
				...
				maxLexicon.trainTree(stateSetTree, randomness, null, secondHalf,false,random);
	    ...
	    Map numbererMap = new HashMap();
			...
			return new ParserData(maxLexicon, maxGrammar, null, numbererMap, numSubStatesArray, opts.verticalMarkovization, opts.horizontalMarkovization, opts.binarization);
...
//						new SophisticatedLexicon(opts.unknownLevel,numSubStatesArray,SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF,smoothParams, new NoSmoothing(),filter,wordNumberer);
					...
					for (Tree<StateSet> stateSetTree : trainStateSetTrees) {
						secondHalf = (n++>nTrees/2.0); 
						lexicon.trainTree(stateSetTree, randomness, null, secondHalf,false,random);
		...
		for (int splitIndex = startSplit; splitIndex < numSplitTimes*3; splitIndex++) {
			...
			if (splitIndex%3==2){//(splitIndex==numSplitTimes*2){
				...
				maxGrammar = maxGrammar.splitAllStates(randomness, counts, allowMoreSubstatesThanCounts, 0, random);
...
//						GrammarMerger.printMergingStatistics(maxGrammar, grammar);
			...
			do {
				...
				double validationLikelihood = GrammarTrainer.calculateLogLikelihood(previousGrammar, previousLexicon, validationStateSetTrees);  // The validation LL of previousGrammar/previousLexicon
			...
			System.out.println(opString+" in the " + (splitIndex/3+1) + "th round is done.");
		}
...
//		maxGrammar.printUnaryRules();
	...
	public static List<Tree<String>> readCorpus(String filename){
		Corpus corpus = new Corpus(filename,TreeBankType.SINGLEFILE,1.0,false);
		return Corpus.binarizeAndFilterTrees(corpus.getTrainTrees(), 1,	0, 10000, Binarization.RIGHT, false,false);
	}
	...
	public static CoarseToFineNBestParser constructNBestParser(ParserData pData){
		...
		CoarseToFineNBestParser parser = new CoarseToFineNBestParser(grammar, lexicon, pData.getNumbs(), opts.kbest,1.0,-1,opts.viterbi,opts.substates,opts.scores, opts.accurate, false, false, true);
		parser.binarization = pData.getBinarization();
		return parser;
	}
	
	public static List<Parse> parseNBest(CoarseToFineNBestParser parser, String sent) {
		ArrayParser.SILENT = true;
		List<Parse> parses = new LinkedList<Parse>();
		try{
			List<String> sentence = Arrays.asList(sent.split(" "));
			List<Tree<String>> parsedTrees = parser.getKBestConstrainedParses(sentence, null, ProductParser.kbest);
	...
	public static Parse parse(ParserData pData, String sent) {
		...
		CoarseToFineMaxRuleParser parser = new CoarseToFineMaxRuleParser(grammar, lexicon, threshold,-1,opts.viterbi,opts.substates,opts.scores, opts.accurate, false, true, true);
	...
	public static List<Parse> parseCorpus(List<String> corpus, ParserData parserData, int nThreads) {
		BerkeleyParser.Options opts = new BerkeleyParser.Options();
		CoarseToFineMaxRuleParser parser = null;
		//if (opts.kbest==1) 
		parser = new CoarseToFineMaxRuleParser(parserData.getGrammar(), parserData.getLexicon(), 1.0,-1,opts.viterbi,opts.substates,opts.scores, opts.accurate, false, true, true);
		...
		MultiThreadedParserWrapper m_parser = new MultiThreadedParserWrapper(parser, nThreads);
