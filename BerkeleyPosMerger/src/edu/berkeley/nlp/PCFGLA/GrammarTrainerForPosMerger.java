...
import constituent.berkeley.POSMerger;
...
public class GrammarTrainerForPosMerger extends edu.berkeley.nlp.PCFGLA.GrammarTrainer {

	public static class SzteOptions extends Options {
		...
		@Option(name = "-splitPosOut", usage = "Output file for splitted preterminals (POS tags), if empty will not splitting")
		public String splitPosOut;

		@Option(name = "-testPath", usage = "Path to test Corpus (Default: null)")
		public String testPath = null;

		@Option(name = "-trainOutPath", usage = "Output path to train Corpus (Default: null)")
		public String trainOutPath = null;

		@Option(name = "-testOutPath", usage = "Output path to test Corpus (Default: null)")
		public String testOutPath = null;

		@Option(name = "-posSplitIter", usage = "Output path to test Corpus (Default: null)")
		public int posSplitIter = 0;

	}

	public static void main(String[] args) {
		OptionParser optParser = new OptionParser(SzteOptions.class);
		SzteOptions opts = (SzteOptions) optParser.parse(args, true);
		...
		List<Tree<String>> validationTrees = Corpus.binarizeAndFilterTrees(
				...
				HORIZONTAL_MARKOVIZATION, maxSentenceLength, binarization,
				manualAnnotation, VERBOSE);
		Numberer tagNumberer = FeatureNumberer.getGlobalNumberer("tags");
		List<Tree<String>> originalTrainTrees = corpus.getTrainTrees();
		...
		StateSetTreeList trainStateSetTrees = new PosMergerStateSetTreeList(
				trainTrees, numSubStatesArray, false, tagNumberer);
		StateSetTreeList validationStateSetTrees = new PosMergerStateSetTreeList(
		...
		/*
		 * double sum = 0; for (Entry<BinaryRule, BinaryRule> e :
		 * grammar.binaryRuleMap.entrySet()) { if (e.getKey().getParentState()
		 * == 4) { System.out.println(e.getKey()); sum +=
		 * e.getKey().scores[0][0][0]; } }
		 * 
		 * for (Entry<UnaryRule, UnaryRule> e : grammar.unaryRuleMap.entrySet())
		 * { if (e.getKey().getParentState() == 4) {
		 * System.out.println(e.getKey()); sum += e.getKey().scores[0][0]; } }
		 * 
		...
		for (int splitIndex = startSplit; splitIndex < numSplitTimes * 3; splitIndex++) {
			...
			if (splitIndex % 3 == 2) {// (splitIndex==numSplitTimes*2){
				...
				trainStateSetTrees = new PosMergerStateSetTreeList(
						trainStateSetTrees, newNumSubStatesArray, false);
		 ...
		 * POS split
		 */
		if (opts.posSplitIter > 0) {

			Corpus testCorpus = null;
			List<Tree<String>> testTrees = null;
			if (opts.testPath != null) {
				testCorpus = new Corpus(opts.testPath, opts.treebank,
						trainingFractionToKeep, false, opts.skipSection,
						opts.skipBilingual, opts.keepFunctionLabels);
				testTrees = testCorpus.trainTrees;
			}

			POSMerger merger = new POSMerger(maxGrammar, maxLexicon,
					trainStateSetTrees, validationStateSetTrees, randomness,
					allowMoreSubstatesThanCounts, smoothParams, filter, nTrees,
					binarization, outFileName, separateMergingThreshold,
					mergingPercentage, opts.rare, opts.splitPosOut,
					originalTrainTrees, testTrees, opts.trainOutPath,
					opts.testOutPath, maxSentenceLength, manualAnnotation);
			merger.splitPos(opts.posSplitIter);
			//merger.splitPOSByFeat(0, false);
			// merger.splitPOSOld(0);
	...
	/*
	 * public static void recalculateTreePoses(StateSetTreeListSzte treeList,
	 * Map<String,Map<String, String>> mergePairs) { for (Tree<StateSet> tree :
	 * treeList) { for (StateSet preTerm : tree.getPreTerminalYield()){ if
	 * (mergePairs.containsKey(preTerm.)) } } }
	 */

	public static Map<String, Map<String, String>> generateMergePairs(
		...
		for (short state = 0; state < grammar.numSubStates.length; state++) {
			if (!grammar.isGrammarTag(state)) {
				String tag = (String) grammar.tagNumberer.object(state);
					...
					for (int j = i + 1; j < grammar.numSubStates[state]; j++) {
						if (mergeThesePairs[state][i][j]) {
							...
							if (mergeMap.containsKey(tag)) {
								mergePair = mergeMap.get(tag);
							} else {
								mergePair = new HashMap<String, String>();
							}
							String feat1 = ((FeatureNumberer) grammar.tagNumberer)
									.getFeatValueIdToName().get((int) state)
									.get(0).get(i);
							String feat2 = ((FeatureNumberer) grammar.tagNumberer)
									.getFeatValueIdToName().get((int) state)
									.get(0).get(j);
							// mergePair.put(feat1, feat1 +
							// Parameters.FeatValueSeparator + feat2);
							// mergePair.put(feat2, feat1 +
							// Parameters.FeatValueSeparator + feat2);
							mergePair.put(feat1, feat2);
							mergeMap.put(tag, mergePair);
						}
					}
				}
			}
...
/*
	public static double doOneEStepForPOS(Grammar previousGrammar,
			Lexicon previousLexicon, Grammar grammar,
			SophisticatedLexiconSzte lexicon,
			StateSetTreeList trainStateSetTrees, boolean updateOnlyLexicon,
			int unkThreshold) {
		boolean secondHalf = false;
		ArrayParser parser = new ArrayParserSzte(previousGrammar,
				previousLexicon);
		// ArrayParser parser = new ArrayParser(previousGrammar,
		// previousLexicon);
		...
		for (Tree<StateSet> stateSetTree : trainStateSetTrees) {
			...
			if ((Double.isInfinite(ll) || Double.isNaN(ll))) {
				...
				lexicon.trainTreeWithPosData(stateSetTree, -1, previousLexicon,
						secondHalf, noSmoothing, unkThreshold, 0);
				if (!updateOnlyLexicon)
					grammar.tallyStateSetTreePOS(stateSetTree, previousGrammar); // E
																					// Step
	...
	/**
	 * Az E és M lépések közötti szakasz módosítása.
	 */
	/*
	public static double doOneEStepModEM(Grammar previousGrammar,
	...
	public static short[] initializeSubStateArray(
		...
		// important
		StateSetTreeList trainStateSetTrees = new PosMergerStateSetTreeList(
				trainTrees, nSub, true, tagNumberer);
		@SuppressWarnings("unused")
		StateSetTreeList validationStateSetTrees = new PosMergerStateSetTreeList(
				validationTrees, nSub, true, tagNumberer);

		PosMergerStateSetTreeList.initializeTagNumberer(trainTrees, tagNumberer);
		PosMergerStateSetTreeList
				.initializeTagNumberer(validationTrees, tagNumberer);
	...
	public static boolean isStatePOS(String state) {
		if (state == null) {
			return false;
		}
		return state.contains("^g");
	}
