...
import constituent.berkeley.RuleSplitters;
...
public class Grammar implements java.io.Serializable {
	...
	public void tallyStateSetTreePOS(Tree<StateSet> tree, Grammar old_grammar) {
		// Check that the top node is not split (it has only one substate)
		if (tree.isLeaf())
			return;
		if (tree.isPreTerminal())
		...
		tallyStateSetTreePOS(tree, tree_score, tree_scale, old_grammar);
	}

	public void tallyStateSetTreePOS(Tree<StateSet> tree, double tree_score,
			double tree_scale, Grammar old_grammar) {
		if (tree.isLeaf())
			return;
		if (tree.isPreTerminal())
		...
		case 1:
			...
			for (short i = 0; i < nChildSubStates; i++) {
				...
				for (short j = 0; j < nParentSubStates; j++) {
					...
					if (tree_score == 0)
						tree_score = 1;
					double logRuleCount = ((PosMergerStateSet)child).weightForFeat(i) * (rS * cIS / tree_score)
							* scalingFactor * pOS;
					ucounts[i][j] += logRuleCount;
				}
			}
			...
			for (short i = 0; i < nLeftChildSubStates; i++) {
				...
				for (short j = 0; j < nRightChildSubStates; j++) {
					...
					for (short k = 0; k < nParentSubStates; k++) {
						...
						if (tree_score == 0)
							tree_score = 1;
						double logRuleCount = ((PosMergerStateSet)leftChild).weightForFeat(i) * ((PosMergerStateSet)rightChild).weightForFeat(j) * (rS * lcIS / tree_score) * rcIS
								* scalingFactor * pOS;
						/*
						 * if (logRuleCount == 0) {
						 * System.out.println("rS "+rS+", lcIS "
	...
	public Grammar splitPOSByFeats(double randomness,
			boolean moreSubstatesThanCounts, int mode, int featId) {
		if (logarithmMode) {
			throw new Error(
					"Do not split states when Grammar is in logarithm mode");
		}
		...
		for (int i = 0; i < numSubStates.length; i++) {
			if (!isGrammarTag(i)) {
				Map<Integer, Map<Integer, String>> feats = ((FeatureNumberer)tagNumberer).getFeatValueIdToName().get(i);
				newNumSubStates[i] = feats.size() > featId ? (short)feats.get(featId).size() : 1;
			} else {
				newNumSubStates[i] = numSubStates[i];
			}
		}
		...
		for (BinaryRule oldRule : binaryRuleMap.keySet()) {
			BinaryRule newRule = RuleSplitters.splitBinaryRuleByPos(oldRule, numSubStates,
					newNumSubStates, random, randomness, doNotNormalize, mode, !isGrammarTag(oldRule.leftChildState), !isGrammarTag(oldRule.rightChildState));
			grammar.addBinary(newRule);
		}

		for (UnaryRule oldRule : unaryRuleMap.keySet()) {
			UnaryRule newRule = RuleSplitters.splitUniaryRuleByPos(oldRule, numSubStates,
					newNumSubStates, random, randomness, doNotNormalize, mode, !isGrammarTag(oldRule.childState));
			grammar.addUnary(newRule);
		}
		grammar.isGrammarTag = this.isGrammarTag;
		grammar.extendSplitTrees(splitTrees, numSubStates);
		...
		for (short i = 0; i < numSubStates[state]; i++) {
			for (short j = (short) (i + 1); j < numSubStates[state]; j++) {
				...
				if (combinedScore != 0 && separatedScoreSum != 0) {
					double log = Math.log(separatedScoreSum
							/ ArrayUtil.sum(combinedScores));
					deltas[state][i][j] += Double.isNaN(log) ? 0.0 : log;
				}
				for (int k = 0; k < 2; k++)
